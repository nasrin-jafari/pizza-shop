...
const CartItem = ({ pizza }) => {
  ...
  return (
    <div className="select-none ">
      ...
      <div
        className="flex flex-wrap items-center gap-3 p-6 border-b
       border-black/10 "
      >
        <div className="font-semibold ">
          Toppings : {pizza.additionalTopping.length === 0 && "None"}
        </div>
        {!!pizza.additionalTopping.length &&
          pizza.additionalTopping.map((topping, index) => (
            <div
              key={index}
              className="capitalize text-sm gradient 
              font-medium px-3 py-1 rounded-full leading-none "
            >
              {topping.name}
            </div>
          ))}
